...
#if MATCH_OPLUS_NFC
	nfc_gpio->vddio_ven = of_get_named_gpio(np, NFC_VDDIO_EN_NAME, 0);
	if ((!gpio_is_valid(nfc_gpio->vddio_ven))) {
		pr_warn("%s: nfc vddio_ven gpio invalid %d\n", __func__, nfc_gpio->vddio_ven);
	}
	nfc_gpio->switch_ven = of_get_named_gpio(np, NFC_SWITCH_EN_NAME, 0);
	if ((!gpio_is_valid(nfc_gpio->switch_ven))) {
		pr_warn("%s: nfc switch_ven gpio invalid %d\n", __func__, nfc_gpio->switch_ven);
	}
	pr_info("%s: irq %d, ven %d, dwl %d, vddio %d, switch %d\n", __func__, nfc_gpio->irq, nfc_gpio->ven,
		nfc_gpio->dwl_req, nfc_gpio->vddio_ven, nfc_gpio->switch_ven);
#else
