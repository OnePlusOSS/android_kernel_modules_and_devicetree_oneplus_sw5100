...
#if MATCH_OPLUS_NFC
void nfc_i2c_dev_shutdown(struct i2c_client *client)
{
	struct nfc_dev *nfc_dev = NULL;
	...
	gpio_set_value(nfc_dev->configs.gpio.ven, 0);
	usleep_range(5000, 5100);    //add for nfc enter off plus mode
	if (gpio_is_valid(nfc_dev->configs.gpio.vddio_ven)) {
		gpio_direction_output(nfc_dev->configs.gpio.vddio_ven, 0);
	}
}
#endif
