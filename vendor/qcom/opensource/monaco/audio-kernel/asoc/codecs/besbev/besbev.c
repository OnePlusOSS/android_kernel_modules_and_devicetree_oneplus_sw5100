...
static int besbev_get_amic_pwr_onoff(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static int besbev_put_amic_pwr_onoff(struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
				snd_soc_kcontrol_component(kcontrol);
	...
	dev_info(besbev->dev, "%s: besbev_put_amic_pwr_onoff: %s \n", __func__, value?"on":"off");

	pdata = besbev_populate_dt_data(besbev->dev);
	if (!pdata) {
	...
	if (value) {
		ret = msm_cdc_enable_ondemand_supply(besbev->dev,
						...
						"cdc-mic-bias");
		if (ret) {
			dev_err(besbev->dev, "%s: amic cdc static supply enable failed!\n",
				__func__);
		} else {
			dev_info(besbev->dev, "%s: amic cdc static supply enable!\n",
				__func__);
		}
	} else {
		ret = msm_cdc_disable_ondemand_supply(besbev->dev,
						...
						"cdc-mic-bias");
		if (ret) {
			dev_err(besbev->dev, "%s: amic cdc static supply disable failed!\n",
				__func__);
		} else {
			dev_info(besbev->dev, "%s: amic cdc static supply disable!\n",
				__func__);
		}
	}
	return 0;
...
static const struct snd_kcontrol_new besbev_amic_snd_controls[] = {
    SOC_SINGLE_EXT("BESBEV AMIC On/Off", SND_SOC_NOPM, 0, 1, 0,
		besbev_get_amic_pwr_onoff,
		besbev_put_amic_pwr_onoff),
};
...
static int besbev_soc_codec_probe(struct snd_soc_component *component)
{
	...
	ret = snd_soc_add_component_controls(besbev->component,
	besbev_amic_snd_controls,ARRAY_SIZE(besbev_amic_snd_controls));
	if (ret < 0) {
		dev_err(component->dev, "%s: Failed to add amic customized snd_ctls\n",
			__func__);
		goto done;
	}
