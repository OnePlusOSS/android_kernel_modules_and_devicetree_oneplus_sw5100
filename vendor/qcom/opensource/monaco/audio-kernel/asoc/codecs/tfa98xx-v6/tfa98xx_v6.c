...
#define TF98XX_FW_NAME_DEFAULT  "tfa98xx.cnt"
#define TF98XX_FW_NAME_21903 	"tfa98xx.21903.cnt"
#define TF98XX_FW_NAME_21905 	"tfa98xx.21905.cnt"
#define TF98XX_FW_NAME_21907 	"tfa98xx.21907.cnt"
#define TF98XX_FW_NAME_22905 	"tfa98xx.22905.cnt"
#define TF98XX_FW_NAME_22907 	"tfa98xx.22907.cnt"


#define TFA98XX_SWITCH_SDM 1
#define TFA98XX_SWITCH_MCU 0
//#endif
...
static bool tfa98xx_is_switch_sdm(struct tfa98xx *tfa98xx);
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx);

extern bool extend_codec_audio_spk_switch_is_locked(void);
extern void set_pa_i2c_name(char *);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!tfa98xx_is_switch_sdm(tfa98xx)) {
		pr_err("tfa98xx %s siwtch is no seleted sdm.\n", __func__);
		return 0;
	}
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("cali_info", S_IRUGO, tfa98xx->dbg_dir,
...
{
	int value = 0;
	if (gpio_is_valid(tfa98xx->sel_gpio)) {
		value = gpio_get_value(tfa98xx->sel_gpio);
		if (value == TFA98XX_SWITCH_SDM) {
			return true;
		}
	}
	return false;
...
#if 0
static int tfa98xx_get_digital_vol(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	error = tfa98xx_get_volume_level(tfa98xx->tfa, &vol);
	if (error != Tfa98xx_Error_Ok) {
		pr_err("[0x%x] get volume error\n", tfa98xx->i2c->addr);
		return -EFAULT;
	}
	pr_debug("Get digital volume level: %d\n", vol);
	ucontrol->value.integer.value[0] = vol;
	return 0;
}

static int tfa98xx_put_digital_vol(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	pr_info("Set digital volume level: %d\n", vol);

	tfa98xx_set_volume_level_v6(tfa98xx->tfa, vol);
	return 0;
}
#endif

static int tfa98xx_dev_stop(struct tfa98xx *tfa98xx)
{
...
static int tfa98xx_get_audio_switch(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	if (gpio_is_valid(tfa98xx->sel_gpio)) {
		value = gpio_get_value(tfa98xx->sel_gpio);
		if (value < 0) {
			dev_err(codec->dev, "cannot get audio sel gpio state, value invalid!\n");
			return -1;
		}
		ucontrol->value.integer.value[0] = value;
		dev_info(codec->dev, "tfa98xx_get_audio_switch : %s\n", value?"sdm":"mcu");
	} else {
		dev_err(codec->dev, "cannot get audio sel gpio state, gpio invalid!\n");
		return -1;
	}
...
static const char * const tfa98xx_switch_text[] = {
	"mcu", "sdm"
};

static int tfa98xx_put_audio_switch(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	dev_info(codec->dev, "tfa98xx_put_audio_switch : %s\n", value?"sdm":"mcu");

	if ((value != 0) && (value != 1)) {
		dev_err(codec->dev, "invalid value : value = %d\n", value);
		return -1;
	}

	if (!strcmp(tfa98xx_switch_text[value], "sdm") &&
		extend_codec_audio_spk_switch_is_locked()) {
		dev_warn(codec->dev, "spk switch is locked, don't switch sdm.\n");
		return 0;
	}

	if (gpio_is_valid(tfa98xx->sel_gpio)) {
		if((!strcmp(tfa98xx_switch_text[value], "mcu"))){
			tfa98xx_dev_stop(tfa98xx);
		}
		gpio_set_value(tfa98xx->sel_gpio, value);
		if (!strcmp(tfa98xx_switch_text[value], "sdm"))  {
			mdelay(50);
			tfa_switch_reg_revert(tfa98xx->tfa);
		}
	} else {
		dev_err(codec->dev, "cannot put audio sel gpio, gpio invalid!\n");
		return -1;
	}
...
static int tfa98xx_get_shutdown(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(codec);
	int value = !(tfa98xx->dsp_init == TFA98XX_DSP_INIT_STOPPED);
...
static const char * const tfa98xx_shutdown_text[] = {
	"Off", "On",
};

static int tfa98xx_put_shutdown(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_component_get_drvdata(codec);
	...
	if((!strcmp(tfa98xx_shutdown_text[value], "On"))){
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING) {
			tfa98xx_dsp_init(tfa98xx);
		} else {
			pr_info("tfa98xx dsp has started\n");
		}
	} else if (!strcmp(tfa98xx_shutdown_text[value], "Off"))  {
		cancel_delayed_work_sync(&tfa98xx->monitor_work);
		cancel_delayed_work_sync(&tfa98xx->init_work);
		mutex_lock(&tfa98xx->dsp_lock);
		if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_STOPPED) {
			pr_info("tfa98xx dsp has beed stoped\n");
			mutex_unlock(&tfa98xx->dsp_lock);
			return 0;
		}
		tfa_dev_stop(tfa98xx->tfa);
...
static const struct soc_enum tfa98xx_switch_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tfa98xx_switch_text),
					tfa98xx_switch_text);
static const struct soc_enum tfa98xx_shutdown_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tfa98xx_shutdown_text),
					tfa98xx_shutdown_text);
...
static const struct snd_kcontrol_new tfa98xx_snd_controls[] = {
	...
	SOC_ENUM_EXT("TFA98XX Audio Switch", tfa98xx_switch_enum,
				tfa98xx_get_audio_switch,
		        tfa98xx_put_audio_switch),
	SOC_ENUM_EXT("TFA98XX Shutdown", tfa98xx_shutdown_enum,
				tfa98xx_get_shutdown,
		        tfa98xx_put_shutdown),
};
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		...
		if (strcmp(fw_name, TF98XX_FW_NAME_DEFAULT)) {
			fw_name = TF98XX_FW_NAME_DEFAULT;
			pr_warn("To loading default fw %s\n", fw_name);
	...
	*/
	...
	// set INTP active high (will output low)
	if (Tfa98xx_Error_Ok != tfa98xx_set_intp_v6(tfa98xx->tfa, 1)) {
		dev_info(tfa98xx->dev, "%s: Failed to set intp\n", __func__);
	} else {
		dev_info(tfa98xx->dev, "%s: set intp to 1 (active high)\n", __func__);
	}
...
{
	int i;
	const char* cnt_name = NULL;
	i = of_property_read_string(tfa98xx->i2c->dev.of_node,"cnt-name", &cnt_name);
	if (i == 0) {
		fw_name = (char*)cnt_name;
	} else {
		fw_name = TF98XX_FW_NAME_DEFAULT;
		pr_info("Failed to read cut-name attribute");
	}
	pr_info("Loading fw %s",fw_name);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			tfa98xx->cstream = 1;
		if (tfa98xx_is_switch_sdm(tfa98xx) != TFA98XX_SWITCH_SDM) {
			pr_err("tfa98xx switch has not seleted sdm !!\n");
			return 0;
		}
		/* Start DSP */
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING) {
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	if (tfa98xx->power_gpio < 0) {
		dev_dbg(dev, "Failed to request power-gpio.\n");
	}

	tfa98xx->sel_gpio = of_get_named_gpio(np, "sel-gpio", 0);
	if (tfa98xx->sel_gpio < 0) {
		dev_dbg(dev, "Failed to request sel-gpio.\n");
	}
	return 0;
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->sel_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, tfa98xx->sel_gpio,
			GPIOF_OUT_INIT_HIGH, "TFA98XX_SEL");
		if (ret)
			return ret;
	}
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		memcpy(pa_i2c_name,(char*)(kobject_name(&(&i2c->dev)->kobj)),7);
		set_pa_i2c_name(pa_i2c_name);
