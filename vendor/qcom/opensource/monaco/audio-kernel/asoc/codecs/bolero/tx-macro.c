...
static const char * const tx_macro_amic_switch_text[] = {
	"mcu", "sdm"
};

static int tx_macro_get_amic_switch(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	int value;
	if (!tx_macro_get_data(component, &tx_dev, &tx_priv, __func__)) {
		dev_err(component->dev, "Failed to get tx_dev,tx_priv.\n");
		return -EINVAL;
	}

	if (gpio_is_valid(tx_priv->sel_gpio)) {
		value = gpio_get_value(tx_priv->sel_gpio);
		if (value < 0) {
		dev_err(component->dev, "cannot get amic sel gpio state, value invalid!\n");
		return -1;
	}
	ucontrol->value.integer.value[0] = value;
	dev_info(component->dev, "tx_macro_get_amic_switch : %s\n", value?"sdm":"mcu");
	} else {
		dev_err(component->dev, "cannot get amic sel gpio state, gpio invalid!\n");
		return -1;
	}
	return 0;
}

static int tx_macro_put_amic_switch(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if ((value != 0) && (value != 1)) {
		dev_err(component->dev, "invalid value : value = %d\n", value);
		return -1;
	}
	...
	dev_info(component->dev, "tx_macro_put_amic_switch : %s\n", value?"sdm":"mcu");

	if (gpio_is_valid(tx_priv->sel_gpio)) {
		if((!strcmp(tx_macro_amic_switch_text[value], "mcu"))){
		//do somethings
		}
		gpio_set_value(tx_priv->sel_gpio, value);
		 if (!strcmp(tx_macro_amic_switch_text[value], "sdm"))  {
		//do somethings
		}
	} else {
		dev_err(component->dev, "cannot put amic sel gpio, gpio invalid!\n");
		return -1;
	}
	return 0;
}
...
static int tx_macro_get_amic_pwr_onoff(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (gpio_is_valid(tx_priv->power_gpio)) {
		value = gpio_get_value(tx_priv->power_gpio);
		if (value < 0) {
			dev_err(component->dev, "cannot get amic sel gpio state, value invalid!\n");
			return -1;
		}
		ucontrol->value.integer.value[0] = value;
		dev_info(component->dev, "tx_macro_get_amic_pwr_onoff : %s\n", value?"on":"off");
	} else {
		dev_err(component->dev, "cannot get amic sel gpio state, gpio invalid!\n");
		return -1;
	}
...
static int tx_macro_put_amic_pwr_onoff(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	dev_info(component->dev, "tx_macro_put_amic_pwr_onoff : %s\n", value?"on":"off");

	if (gpio_is_valid(tx_priv->power_gpio)) {
		gpio_set_value(tx_priv->power_gpio, value);
	} else {
		dev_err(component->dev, "cannot put amic sel gpio, gpio invalid!\n");
		return -1;
	}
...
static const struct soc_enum tx_macro_amic_switch_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tx_macro_amic_switch_text),
		tx_macro_amic_switch_text);


static const struct snd_kcontrol_new tx_macro_amic_snd_controls[] = {
	SOC_ENUM_EXT("AMIC Switch", tx_macro_amic_switch_enum,
		tx_macro_get_amic_switch,
		tx_macro_put_amic_switch),
	SOC_SINGLE_EXT("AMIC PWR On/Off", SND_SOC_NOPM, 0, 1, 0,
		tx_macro_get_amic_pwr_onoff,
		tx_macro_put_amic_pwr_onoff),
};
...
static int tx_macro_gpio_init(struct tx_macro_priv *tx_priv)
{
	struct device *dev = tx_priv->dev;
	...
	if (!np) {
		tx_priv->power_gpio = -1;
		tx_priv->sel_gpio = -1;
		dev_err(dev, "devi of_node is NULL\n");
		return 0;
	}

	tx_priv->power_gpio = of_get_named_gpio_flags(np, "power-gpio", 0, &flags);
	if (gpio_is_valid(tx_priv->power_gpio)) {
		gpio_low = !!(flags & OF_GPIO_ACTIVE_LOW);
		dev_info(dev, "amic power-polarity:%d\n", !gpio_low);

		ret = devm_gpio_request_one(dev, tx_priv->power_gpio,
			gpio_low ? GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH,
			"AMIC_PWR");
		if (ret) {
			tx_priv->power_gpio = -1;
		...
		dev_err(dev, "No power GPIO provided, will not HW power ctrl\n");

	tx_priv->sel_gpio = of_get_named_gpio_flags(np, "sel-gpio", 0, &flags);
	if (gpio_is_valid(tx_priv->sel_gpio)) {
		gpio_low = !!(flags & OF_GPIO_ACTIVE_LOW);
		dev_info(dev, "amic sel-polarity:%d\n", !gpio_low);

		ret = devm_gpio_request_one(dev, tx_priv->sel_gpio,
			gpio_low ? GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH,
			"AMIC_SEL");
		if (ret) {
			tx_priv->sel_gpio = -1;
			dev_warn(dev, "Failed to request sel-gpio\n");
		}
	} else
		dev_err(dev, "No sel GPIO provided, will not HW switch select\n");
...
static int tx_macro_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_add_component_controls(component,
		tx_macro_amic_snd_controls,ARRAY_SIZE(tx_macro_amic_snd_controls));
	if (ret < 0) {
		dev_err(tx_dev, "%s: Failed to add amic customized snd_ctls\n",
			__func__);
		return ret;
	}
