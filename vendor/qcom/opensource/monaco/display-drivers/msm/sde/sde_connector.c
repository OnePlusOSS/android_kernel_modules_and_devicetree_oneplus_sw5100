...
#if 0/*vendor add, enter idle need enable esd check thread*/
	mutex_unlock(&c_conn->lock);
...
void sde_connector_helper_bridge_disable(struct drm_connector *connector)
{
	...
	if (!sde_in_trusted_vm(sde_kms) && c_conn->bl_device && !poms_pending) {
		...
		if (!display->panel->aod_sw)
			backlight_update_status(c_conn->bl_device);
		else
			SDE_INFO("backlight_update none\n");
	}
...
int sde_connector_esd_status(struct drm_connector *conn)
{
	...
	/*VENDOR_EDIT add for esd check,mipi switch is missing
	and display is still on, skip esd check, return true*/
	if (!get_switch_status_by_id(SWITCH_LCD)) {
		SDE_ERROR("ESD check missing switch\n");
		return ret;
	}
	/*VENDOR_EDIT end*/
	...
	if (ret <= 0) {
		...
		/* #VENDOR_EDIT report panel dead need hw reset*/
		display->panel->hw_rst = 1;
		/* #endif */
...
static void sde_connector_check_status_work(struct work_struct *work)
{
	...
	/* #VENDOR_EDIT reset panel and report panel dead*/
	display = (struct dsi_display *)conn->display;
	display->panel->hw_rst = 1;

#ifdef OPLUS_TODO//todo
	report_cnt ++;
	if(report_cnt < MAX_PRT_CNT) {
		oplus_kevent_fb(FB_DISPLAY, "display_esd",
			"display esd chk failed");
	}
	/* #endif */
#endif
