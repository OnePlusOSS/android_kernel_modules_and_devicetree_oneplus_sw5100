...
extern void set_display_vflip(uint value);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtouint(buf, 0, &val)) {
		SDE_ERROR("kstrtouint buf error!\n");
		return -EINVAL;
	}

	if (val == 1 && (g_dsi_display->panel->bl_config.bl_level < 255)){
		SDE_ERROR("enable hbm error! current bl:%d\n",
					g_dsi_display->panel->bl_config.bl_level);
		return count;
	}

	if (g_dsi_display->panel->power_mode == SDE_MODE_DPMS_OFF){
		SDE_ERROR("enable hbm error! display state is off\n");
		return count;
	}

	mutex_lock(&hbm_lock);
	if(val == 1 && (g_dsi_display->panel->is_hbm_enabled == 0)){
		rc = dsi_display_set_hbm(g_dsi_display, true);
		g_dsi_display->panel->is_hbm_enabled = 1;
	}
	else if (val == 0 && (g_dsi_display->panel->is_hbm_enabled == 1)) {
		if (g_dsi_display->panel->bl_config.bl_level != 0)
			rc = dsi_display_set_hbm(g_dsi_display, false);
		g_dsi_display->panel->is_hbm_enabled = 0;
	}
	else
		SDE_INFO("error val %d or is_hbm_enabled:%d, do nothing!\n",
				val, g_dsi_display->panel->is_hbm_enabled);

	SDE_INFO("set hbm %d, rc:%d, mode:%d\n", val, rc,
		g_dsi_display->panel->power_mode);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->is_hbm_enabled);
}

static ssize_t panel_rst_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int val;
	...
	if (val == 1) {
		g_dsi_display->panel->rst_flag = val;
		/*switch from mcu, should enable lcd power and rst first*/
	...
	{
		g_dsi_display->panel->rst_flag = 0;
	}
	SDE_INFO("set panel rst disable,flag %d\n",val);

	return count;
}

static ssize_t panel_rst_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	struct drm_crtc *crtc;
	struct sde_crtc *sde_crtc;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	SDE_INFO("set idle state:%d\n", val);
	if(val == 1)
	{
		dsi_panel_set_lp1(g_dsi_display->panel);
	}
	else if (val == 0)
	{
		dsi_panel_set_nolp(g_dsi_display->panel);
	}
	else
		SDE_ERROR("idle error val %d, do nothing!\n",val);
...
static ssize_t cmd_w_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	char databuf[20];

	memset(databuf, 0, 20);
	if (sscanf(buf,"%x %x %x %x %x %x %x %x %x", &databuf[0], &databuf[1],
			&databuf[2], &databuf[3], &databuf[4],
			&databuf[5], &databuf[6], &databuf[7],
			&databuf[8]) != 9) {
		SDE_ERROR("sscanf buf error! %s, 0x%x, 0x%x\n",
				buf, databuf[0], databuf[1]);
		return count;
    }
#if 0
	dsi_panel_cmd_write(g_dsi_display->panel,
		(uint8_t)databuf[0],
		databuf[1]);
	dsi_display_cmd_write(g_dsi_display,
		databuf,
		9);
#endif
	return count;
...
static ssize_t cmd_w_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	SDE_INFO("hello world!\n");
	return 0;
}

static ssize_t panel_pwr_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_crtc *crtc;
	unsigned int val;
	...
	if (val == 0) {
		/*close tp click on display */
		if (IS_PANEL_FT2389(oplus_panel_id)) {
			g_dsi_display->panel->pwr_flag = 0;
		}
	}
	else if (val == 1) {
		/*flag 1 add for close display switch to mcu*/
		//rc = dsi_display_pre_disable(g_dsi_display);
		rc = dsi_display_disable(g_dsi_display);
		g_dsi_display->panel->pwr_flag = 1;
	}
	else if ( val == 2) {
		/*flag 2 add for tp click on display */
		if (IS_PANEL_FT2389(oplus_panel_id)) {
			g_dsi_display->panel->pwr_flag = 2;
			SDE_INFO("set display keep power\n");
		}
	}
	else {
		//rc = dsi_display_prepare(g_dsi_display);
	}
	SDE_INFO("set display power mode %d\n",val);

	return count;
}

static ssize_t panel_pwr_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->pwr_flag);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 2)  /*add for mmi close dimming when backlight cailbration*/
		dsi_panel_data_dimming_off(g_dsi_display->panel);

	if (val == 1 || val == 0) {
		g_dsi_display->panel->dimming_enable = val;
	}
	else {
		g_dsi_display->panel->dimming_enable = 0;
	}
	SDE_INFO("set display dimming enable %d\n",val);
...
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->dimming_enable);
}

static ssize_t panel_aod_sw_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int val;
	if (kstrtouint(buf, 0, &val)) {
	...
	if (val == 1 || val == 0) {
		g_dsi_display->panel->aod_sw = val;
	}
	else {
		g_dsi_display->panel->aod_sw = 0;
	}
	SDE_INFO("set display aod sw %d\n",val);

	return count;
}

static ssize_t panel_aod_sw_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->aod_sw);
}

static ssize_t aod_bl_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int bl_idle_on = 0;
	unsigned int bl_idle_off = 0;

	if (sscanf(buf, "%d %d", &bl_idle_on, &bl_idle_off) != 2) {
		SDE_ERROR("sscanf input error!\n");
		return -EINVAL;
	}
	g_dsi_display->panel->enter_idle_bl = bl_idle_on;
	g_dsi_display->panel->exit_idle_bl = bl_idle_off;

	SDE_INFO("set display bl on:%d, off:%d\n",
			bl_idle_on, bl_idle_off);
	return count;
}

static ssize_t aod_bl_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->enter_idle_bl);
}

static ssize_t panel_recovery_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int val;
	if (kstrtouint(buf, 0, &val)) {
	...
	SDE_INFO("try panel recovery %d\n",val);
	return count;
}
static ssize_t panel_recovery_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->hw_rst);
}

static ssize_t force_pwroff_store(struct device *device,
		struct device_attribute *attr, const char *buf, size_t count)
{
#if 0 /*disbale display power*/
	gpio_direction_output(DISP_RST, 0);
	gpio_direction_output(DISP_LDO_3P3, 0);
	gpio_direction_output(DISP_1P8, 0);
#endif
	gpio_direction_output(DISP_POWER_IC_3p3, 0);  /*close panel power ic*/
	if (g_dsi_display->panel->power_mode == SDE_MODE_DPMS_OFF)
		g_dsi_display->panel->panel_initialized = false;

	SDE_INFO("force power off panel io, mode:%d, init:%d\n",
				g_dsi_display->panel->power_mode,
				g_dsi_display->panel->panel_initialized);
	return count;
}

static ssize_t force_pwroff_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->hw_rst);
...
static ssize_t power_state_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n",
			g_dsi_display->panel->power_state);
}
...
static DEVICE_ATTR_RW(panel_rst);
static DEVICE_ATTR_RW(idle);
static DEVICE_ATTR_RW(cmd_w);
static DEVICE_ATTR_RW(panel_pwr);
static DEVICE_ATTR_RW(dimming);
static DEVICE_ATTR_RW(panel_aod_sw);
static DEVICE_ATTR_RW(aod_bl);
static DEVICE_ATTR_RW(panel_recovery);
static DEVICE_ATTR_RW(force_pwroff);
...
static struct attribute *sde_crtc_dev_attrs[] = {
	...
	&dev_attr_hbm.attr,
	&dev_attr_panel_rst.attr,
	&dev_attr_idle.attr,
	&dev_attr_cmd_w.attr,
	&dev_attr_panel_pwr.attr,
	&dev_attr_dimming.attr,
	&dev_attr_panel_aod_sw.attr,
	&dev_attr_aod_bl.attr,
	&dev_attr_panel_recovery.attr,
