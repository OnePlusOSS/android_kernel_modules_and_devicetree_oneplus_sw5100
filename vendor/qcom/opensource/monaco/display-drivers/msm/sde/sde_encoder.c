		...
		struct sde_encoder_wait_info *wait_info)
{
	...
	/* VENDOR_EDIT mipi switch is missing when display is still on*/
	if (!get_switch_status_by_id(SWITCH_LCD)) {
		SDE_INFO("mipi switch to mcu, ret:%d\n", ret);
		ret = 1;
	}
	/*end add*/
	if (ret <= 0) {
...
void sde_encoder_helper_trigger_start(struct sde_encoder_phys *phys_enc)
{
	...
	if (ctl && ctl->ops.trigger_start) {
		if (!g_dsi_display->panel->rst_flag) {
			ctl->ops.trigger_start(ctl);
		} else {
			SDE_INFO("skip trigger_start black rst_flag:%d\n",
