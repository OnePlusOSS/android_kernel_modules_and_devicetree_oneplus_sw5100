...
#include <oplus/msm_drm_notify.h>
...
#include <oplus/oplus_switch.h>
#include <linux/soc/qcom/panel_event_notifier.h>
//#include "oplus/boot_mode.h"
/**
...
#define DEFAULT_MINI_BL        0x4F  /*1.43 panel, default level 2 brightness*/
unsigned int oplus_panel_id = PANEL_TM_ICNA3311_143;

extern struct sde_connector * g_conn;
extern struct dsi_display *g_dsi_display;
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->vci3p3_en_gpio)) {
		rc = gpio_request(r_config->vci3p3_en_gpio, "vci3p3_en_gpio");
		if (rc) {
			DSI_ERR("request for vci3p3_en_gpio failed, rc=%d\n", rc);
			goto error_release_vci3p3_en;
		}
	}
...
error_release_vci3p3_en:
	if (gpio_is_valid(r_config->vci3p3_en_gpio))
		gpio_free(r_config->vci3p3_en_gpio);
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.vci3p3_en_gpio)) {
		rc = gpio_direction_output(panel->reset_config.vci3p3_en_gpio, 1);
		if (rc) {
			DSI_ERR("unable to set dir for vci 3p3 gpio rc=%d\n", rc);
			goto exit;
		}
	}
...
/*VENDOR_EDIT */
void dsi_panel_tp_hw_reset(void)
{
	/*check switch first*/
	if (!get_switch_status_by_id(SWITCH_LCD)) {
		DSI_ERR("need hw reset panel but no hw connect!\n");
		return ;
	}

	//gpio_direction_output(TP_RST, 0);
	gpio_direction_output(DISP_RST, 0);

	//gpio_direction_output(OTS_3P0, 0);
	gpio_direction_output(DISP_POWER_IC_3p3, 0);
	//gpio_direction_output(DISP_LDO_3P3, 0);
	gpio_direction_output(DISP_1P8, 0);

	usleep_range(20000, 21000);

	gpio_direction_output(DISP_1P8, 1);
	usleep_range(5000, 5100);

	//gpio_direction_output(OTS_3P0, 1);
	//gpio_direction_output(DISP_LDO_3P3, 1);
	gpio_direction_output(DISP_POWER_IC_3p3, 1);

	usleep_range(80000, 81000); /*at least 4 frames time = 16.6 * 4 = 66.4*/
	gpio_direction_output(DISP_RST, 1);
	//gpio_direction_output(TP_RST, 1);

	DSI_INFO("hw reset panel\n");
}

void dsi_event_notify_panel_dead(struct sde_connector *conn)
{
...
{
	dsi_event_notify_panel_dead(g_conn);
	g_dsi_display->panel->hw_rst = 1;
}
EXPORT_SYMBOL(display_pwr_down_try_recovery);
/*VENDOR_EDIT endif */
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	/* VENDOR_EDIT optimization of power-on sequence for vddio(1p8) and avdd(3p3)*/
	...
	if (rc) {
	 ...
	 /* VENDOR_EDIT skip reset panel when framework get power on panel form mcu*/
	if (panel->rst_flag) {
		DSI_INFO("skip reset panel\n");
		//panel->rst_flag = 0;
		goto exit;
	}
...
error_disable_gpio:
	...
	if (gpio_is_valid(panel->reset_config.vci3p3_en_gpio))
		gpio_set_value(panel->reset_config.vci3p3_en_gpio, 0);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	/*used for recovery panel*/
	if (panel->hw_rst) {
		if (IS_PANEL_FT2389(oplus_panel_id)) {
			dsi_panel_tp_hw_reset();
			DSI_INFO("power down panel\n");
		}
		panel->hw_rst = 0;	/*clear flag*/
	}

	usleep_range(3000, 3100);
	 /*add for touch on display, keep ldo_3p3 and reset on*/
	if (panel->pwr_flag != 2) {
	...
	if (panel->pwr_flag != 2) {
		rc = dsi_panel_set_pinctrl_state(panel, false); /*pmic gpio1 avdd_3p3*/
		if (rc) {
			DSI_ERR("[%s] failed set pinctrl state, rc=%d\n", panel->name,
				rc);
		}
	}
	/*add delay for display power seq,  reset first and then 1p8, 3p*/
	usleep_range(3000, 3100);
...
//#VENDOR_EDIT
static int dsi_panel_hbm_handle(struct dsi_panel *panel,
	u32 bl_lvl)
{
	int rc = 0;
	...
	/*dimming step for exit/enter HBM for NT37350*/
		/*bl up to hbm*/
	if (bl_lvl >= 255) {
			/* 63H = 00, 51H = FF*/
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFF, 0x10 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFB, 0x01 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x63, 0x00 }, 2);  /*hbm brightness*/
	}
	return rc;
}
//#endif
	...
	u32 bl_lvl)
{
	...
	bl = bl_lvl;
	/*VENDOR_EDIT for dimming ctrl*/
	if (panel->dimming_enable &&
		!panel->is_dimming_enabled)
		dsi_panel_data_dimming_on(panel);

	if (!panel->dimming_enable &&
		panel->is_dimming_enabled)
		dsi_panel_data_dimming_off(panel);
	/*end add*/

	/*dimming step for exit/enter HBM for NT37350*/
	if (PANEL_VISIONOX_NT37350_174 == oplus_panel_id) {
		dsi_panel_hbm_handle(panel, bl_lvl);
	...
	DSI_INFO("update dcs backlight:%d\n", bl);

	if (unlikely(panel->bl_config.lp_mode))
		dsi->mode_flags = mode_flags;

	/*VENDOR_EDIT for HBM*/
	if (panel->is_hbm_enabled && bl == 255)
		dsi_panel_hbm_on(panel);
	/*VENDOR_EDIT end*/
	return rc;
}
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-lock-command-state",
	"qcom,mdss-dsi-unlock-command-state"
};
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->reset_config.disp_en_gpio)) {
		...
		panel->reset_config.vci3p3_en_gpio =
				utils->get_named_gpio(utils->data,
					"qcom,platform-vci3p3-en-gpio", 0);
...
#ifdef OPLUS_TODO
	if((boot_mode == MSM_BOOT_MODE__WLAN) ||
		 (boot_mode == MSM_BOOT_MODE__RF) ||
		 (boot_mode == MSM_BOOT_MODE__FACTORY)) {
		DSI_INFO("current is wlan or ftm mode:%d\n",boot_mode);
	} else {
...
static int dsi_panel_enter_idle_cmd(struct dsi_panel *panel)
{
	int rc = 0;
	struct mipi_dsi_device *dsi = NULL;
	u32 bl_lvl = 0;
	...
	/*current mode is hbm, exit hbm first, for FT2389*/
	if ((panel->is_hbm_enabled) &&
		(IS_PANEL_FT2389(oplus_panel_id))) {
		dsi_panel_hbm_off(panel);
		panel->is_hbm_enabled = 0;
	}

	dsi = &panel->mipi_device;
	bl_lvl = panel->enter_idle_bl;
	if (bl_lvl == 0)
		bl_lvl = 0x60;

	if (IS_PANEL_FT2389(oplus_panel_id)) {
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x26, 0x01 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x39, 0x00 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x51, bl_lvl}, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x69, 0xD0 }, 2);
	}
	if (oplus_panel_id == PANEL_VISIONOX_NT37350_174) {
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFF, 0x10 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFB, 0x01 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x51, bl_lvl}, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x39, 0x00 }, 2);
	...
	panel->idle_en = 1;
	DSI_INFO("set idle bl %d\n", bl_lvl);
	return rc;
}

static int dsi_panel_exit_idle_cmd(struct dsi_panel *panel, bool with_bl)
{
	int rc = 0;
	struct mipi_dsi_device *dsi = NULL;
	u32 bl_lvl = 0;
	...
	dsi = &panel->mipi_device;
	bl_lvl = panel->exit_idle_bl;
	if (bl_lvl == 0)
		bl_lvl = DEFAULT_MINI_BL;

	if (IS_PANEL_FT2389(oplus_panel_id)) {
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x69, 0x00 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x38, 0x00 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0x26, 0x00 }, 2);
	}
	if (oplus_panel_id == PANEL_VISIONOX_NT37350_174) {
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFF, 0x10 }, 2);
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFB, 0x01 }, 2);
	...
	panel->idle_en = 0;
	DSI_INFO("set idle off bl %d\n", bl_lvl);
	return rc;
}

int dsi_panel_rotate_enable(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	struct mipi_dsi_device *dsi = NULL;
	...
	if (IS_PANEL_ICNA3311(oplus_panel_id)) {
		rc = mipi_dsi_generic_write(dsi, (u8[]){0xFE, 0x00 }, 2);

		if (enable) {
			rc = mipi_dsi_generic_write(dsi, (u8[]){0x36, 0x40 }, 2); /*left and right flip*/
			rc = mipi_dsi_generic_write(dsi, (u8[]){0x2a, 0x00, 0x08, 0x01, 0xD9}, 5); /*start colum addr, right from 8 to 473*/
		} else {
			rc = mipi_dsi_generic_write(dsi, (u8[]){0x36, 0x00 }, 2); /*0 flip*/
			rc = mipi_dsi_generic_write(dsi, (u8[]){0x2a, 0x00, 0x06, 0x01, 0xD7}, 5); /*start colum addr, left from 6 to 471*/
		}
	}

	DSI_INFO("set panel rotate enable:%d\n",enable);
	return rc;
}
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	if ((oplus_panel_id == PANEL_VISIONOX_NT37350_174) ||
		IS_PANEL_FT2389(oplus_panel_id)) {
		dsi_panel_enter_idle_cmd(panel);
	}
	else
	{
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	/*VENDDIR_EDIT*/
	oplus_panel_event_data_notifier_trigger(panel,
				DRM_PANEL_EVENT_UNBLANK, 1, false);
	/*VENDDIR_EDIT end*/
	DSI_INFO("exit idle");
	if (panel->is_twm_en) {
	...
	if (IS_PANEL_FT2389(oplus_panel_id) ||
		(oplus_panel_id == PANEL_VISIONOX_NT37350_174)) {
		dsi_panel_exit_idle_cmd(panel, true);
	}
	else {
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	if (get_display_vflip())
		dsi_panel_rotate_enable(panel, true);
error:
	mutex_unlock(&panel->panel_lock);
	if (panel->rst_flag)
		dsi_panel_cmd_unlock(panel);
...
int dsi_panel_post_unprepare(struct dsi_panel *panel)
{
	...
	if (panel->aod_sw) {
		DSI_INFO("aod switch display\n");
		panel->aod_sw = 0;
		panel->power_state = SDE_MODE_DPMS_OFF;
		return rc;
	}
	mutex_lock(&panel->panel_lock);
...
int dsi_panel_hbm_on(struct dsi_panel *panel)
{
	...
	DSI_INFO("[%s] HBM enable bl:%d", __func__,
	...
	/*if current is idle mode, exit it*/
	if (panel->idle_en &&
		IS_PANEL_FT2389(oplus_panel_id)) {
		dsi_panel_exit_idle_cmd(panel, false);
	}

	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_ON);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_HBM_ON cmds, rc=%d\n",
		       panel->name, rc);
	}
...
//EXPORT_SYMBOL(dsi_panel_hbm_on);
...
int dsi_panel_hbm_off(struct dsi_panel *panel)
{
	...
	DSI_INFO("[%s] HBM disable bl:%d", __func__,
				panel->bl_config.bl_level);

	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_OFF);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_HBM_OFF cmds, rc=%d\n",
		       panel->name, rc);
	}
...
//EXPORT_SYMBOL(dsi_panel_hbm_off);

int dsi_panel_data_dimming_on(struct dsi_panel *panel)
{
	int rc = 0;
	...
	DSI_INFO("%s enter\n", __func__);
	//mutex_lock(&panel->panel_lock);
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_DATA_DIMMING_ON);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_DATA_DIMMING_ON cmds, rc=%d\n",
		       panel->name, rc);
	}
	panel->is_dimming_enabled = 1;
	//mutex_unlock(&panel->panel_lock);
	return rc;
}

int dsi_panel_data_dimming_off(struct dsi_panel *panel)
{
	int rc = 0;
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_DATA_DIMMING_OFF);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_DATA_DIMMING_OFF cmds, rc=%d\n",
		       panel->name, rc);
	}
	panel->is_dimming_enabled = 0;
	//mutex_unlock(&panel->panel_lock);
...
int dsi_panel_cmd_unlock(struct dsi_panel *panel)
{
	int rc = 0;
	...
	DSI_INFO("%s enter\n", __func__);
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CMD_UNLOCK);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_DATA_DIMMING_ON cmds, rc=%d\n",
		       panel->name, rc);
	}
		...
		bool early_trigger)
{
	...
	DSI_DEBUG("[%s] type=0x%X, data=%d, early_trigger=%d\n",
			panel->type, notif_type, data, early_trigger);

	memset(&notifier, 0, sizeof(notifier));
	notifier.panel = (void *)OPLUS_PANEL_EVENT_MAGIC_NUM;
