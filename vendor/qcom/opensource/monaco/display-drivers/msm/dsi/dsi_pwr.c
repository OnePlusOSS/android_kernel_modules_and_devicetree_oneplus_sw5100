...
static int dsi_pwr_enable_vregs(struct dsi_regulator_info *regs, bool enable)
{
	...
	if (enable) {
		for (i = 0; i < regs->count; i++) {
			...
			/*add for touch on display, vddio_1p8 is on*/
			if ((strncmp(vreg->vreg_name, "vddio", 5) == 0)) {
				if(g_dsi_display && g_dsi_display->panel->vddio_enabled) {
					 DSI_INFO("skip enable vddio_1p8");
					continue;
				}
			}
		...
		for (i = (regs->count - 1); i >= 0; i--) {
			...
			/*add for touch on display, keep vddio_1p8 on*/
			if((strncmp(vreg->vreg_name, "vddio", 5) == 0)) {
				if (g_dsi_display) {
					if (g_dsi_display->panel->pwr_flag == 2) {
						g_dsi_display->panel->vddio_enabled = 1;
						DSI_INFO("skip close vddio_1p8");
						continue;
					} else {
						g_dsi_display->panel->vddio_enabled = 0;
					}
				}
			}
