...
#define DISPLAY_SELF_DIAG_REG     0x0F
#define ERROR_SELF_DIAG_RESULT    0x01
#define ERROR_SELF_DIAG_SLPOUT_RESULT  0xC1
		...
		void *display, u32 bl_lvl)
{
	...
	DSI_DEBUG("bl_scale = %u, bl_scale_sv = %u, bl_lvl = %u rst_flag = %d\n",
		bl_scale, bl_scale_sv, (u32)bl_temp, panel->rst_flag);

	if (panel->rst_flag && (bl_temp == 0)) {
		panel->rst_flag = 0;
		DSI_INFO("skip set backlight to 0\n");
	} else {
		panel->rst_flag = 0;
...
/****vendor edit****/
static int dsi_display_cmd_read(struct dsi_display *display,
		u8 reg, u8 *value)
{
	int i, rc = 0, count = 0, start = 0, *lenp;
	struct drm_panel_esd_config *config;
	...
	if (!display->panel || !ctrl || !ctrl->ctrl || !display)
		return -EINVAL;
	...
	memcpy(&esd_config, &(panel->esd_config),
			sizeof(struct drm_panel_esd_config));

	//config = &(panel->esd_config);
	config = &esd_config;
	lenp = config->status_valid_params ?: config->status_cmds_rlen;
	count = config->status_cmd.count;
	memcpy(&cmds_new, config->status_cmd.cmds,
...
static int check_display_self_diagnostic_result(struct dsi_display *display)
{
	int rc = 0x1;
	u8 value = 0;

	rc = dsi_display_cmd_read(display,
			DISPLAY_SELF_DIAG_REG, &value);

	 /*if SRAM checksum error (default is 0xC0, 0x00)
	  *need report panel dead, try esd recovery*/
	if (value == ERROR_SELF_DIAG_RESULT ||
		value == ERROR_SELF_DIAG_SLPOUT_RESULT) {
		DSI_ERR("SRAM checksum err, try recovery! value:%x\n",value);
		rc = 0x0;
	}
	return rc;
}

int dsi_display_set_hbm(struct dsi_display *display, bool en)
{
	struct dsi_panel *panel;
	int rc = 0;
...
static int dsi_display_status_reg_read(struct dsi_display *display)
{
	...
	if (!get_switch_status_by_id(SWITCH_LCD)) {
		DSI_INFO("panel not connect! no check\n");
		rc = 1;
		return rc;
	}
	m_ctrl = &display->ctrl[display->cmd_master_idx];
					...
					bool te_check_override)
{
	...
	if (status_mode == ESD_MODE_REG_READ) {
		/* if one of 'status (0x0A)' or 'self-diagnostic result(0x0F)'
		 *check faild, need recovery */
		rc = dsi_display_status_reg_read(dsi_display);
		if (rc > 0) {
			rc = check_display_self_diagnostic_result(dsi_display);
		}
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	/*VENDDIR_EDIT*/
	display->panel->power_state = SDE_MODE_DPMS_ON;
	oplus_panel_event_data_notifier_trigger(display->panel,
			DRM_PANEL_EVENT_UNBLANK, 0, false);
	DSI_INFO("poweron display\n");
	/*VENDDIR_EDIT end*/
...
int dsi_display_disable(struct dsi_display *display)
{
	...
	/*VENDDIR_EDIT*/
	oplus_panel_event_data_notifier_trigger(display->panel,
			DRM_PANEL_EVENT_BLANK, 0, false);
	DSI_INFO("poweroff display mode:%d, init:%d\n",
