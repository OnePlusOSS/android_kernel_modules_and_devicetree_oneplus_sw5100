...
if [ -n "${SKIP_IF_VERSION_MATCHES}" ]; then
  if [ -f "${DIST_DIR}/vmlinux" ]; then
    ...
    if [[ ! "$kernelversion" =~ .*dirty.* ]] && \
       grep -o -a -m5 "Linux version [^ ]* " ${DIST_DIR}/vmlinux | grep -q " ${kernelversion} " ; then
      echo "========================================================"
...
if [ -n "${ABL_SRC}" ]; then
  if [ -e "${ROOT_DIR}/${ABL_SRC}" ]; then
    if [ -n "${MSM_ARCH}" ]; then
      ...
      #Clean abl
      if [ -e "${COMMON_OUT_DIR}/abl-${TARGET_BUILD_VARIANT}" ]; then
        echo " Clean abl temporary files"
        rm -rf ${COMMON_OUT_DIR}/abl-${TARGET_BUILD_VARIANT}
      fi
